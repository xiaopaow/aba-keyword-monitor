...
export default function KeywordsPage() {
  ...
  useEffect(() => {
    fetchKeywords().then((data) => setRows(data.rows));
  }, []);
      ...
      { accessorKey: "tag", header: "标签", cell: ({ row }) => row.original.tag ? <Badge tone="blue">{row.original.tag}</Badge> : "-" },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <button aria-label="收藏" title="收藏" className="rounded p-1 hover:bg-slate-100 dark:hover:bg-slate-800">
  ...
  return (
    <>
      ...
      <Filters>
        <Field label="关键词搜索"><input className={inputClass} placeholder="wireless earbuds" /></Field>
        ...
        <div className="flex items-end">
          <Button className="w-full"><Search className="h-4 w-4" />搜索</Button>
        </div>
      </Filters>
